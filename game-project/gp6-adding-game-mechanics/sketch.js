...
function draw()
{
	...
	//The game character
	if(isLeft && isFalling) {drawGameCharJumpingLeft();}
	else if(isRight && isFalling) {drawGameCharJumpingRight();}
	else if(isLeft) {drawGameCharWalkingLeft();}
	else if(isRight) {drawGameCharWalkingRight();}
	else if(isFalling || isPlummeting) {drawGameCharFrontFacingJumping()}
 	else {drawGameCharStandingFrontFacing();}
	...
	if (flagpole.isReached) {
		fill(255);
		textSize(50);
		text("Level Complete", 512 - 130, 288);
	}
...
function startGame() {
	...
	gameWorldEnding = 1800

	flagpole = {x_pos: gameWorldEnding, y_pos: 317, isReached: false}
